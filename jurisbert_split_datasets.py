...
def plot_confusion_matrix(y_true, y_pred, fold_number=None):
    conf_matrix = confusion_matrix(y_true, y_pred)
    plt.figure(figsize=(6, 4))
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='viridis', xticklabels=['Banco', 'Cliente'], yticklabels=['Banco', 'Cliente'])
    plt.xlabel('Predito')
    plt.ylabel('Real')
...
for fold_number, (train_index, val_index) in enumerate(kf.split(train_val_df)):
    ...
    print(f"Acuracia: {results['eval_accuracy']} F1: {results['eval_f1']} Prec: {results['eval_precision']} Recall: {results['eval_recall']}")
...
print(f"\nTeste - Acuracia: {test_results['eval_accuracy']} F1: {test_results['eval_f1']} Prec: {test_results['eval_precision']} Recall: {test_results['eval_recall']}")
